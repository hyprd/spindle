...
void graph_print(graph g){
    ...
    for(u = 0; u < g -> capacity; u++){
        printf("%d\t%d\t%d\t%d\n", u, g -> vertices[u].dist, g -> vertices[u].predecessor, g -> vertices[u].finish);
    }
}
/* 
...
/* 
 * Recursively visits a vertex within a graph.
...
graph graph_dfs(graph g){
    ...
    for(e = 0; e < g -> capacity; e++){
        g -> vertices[e].state = UNVISITED;
        g -> vertices[e].predecessor = -1;
    }
    step = 0;
    for(v = 0; v < g -> capacity; v++){
