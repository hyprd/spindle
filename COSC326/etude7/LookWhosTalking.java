...
public class LookWhosTalking {
    ...
    private static String getVerb(String[] words) {
        ...
        if (dict.containsKey(verb)) return dict.get(verb);
        return null;
    }

    private static Person getPerson(String[] words) {
        // First person singular
        if (words[0].equals("i")) return Person.FIRSTEXCL;

        // Accounts for "you two"
        if (words[1].equals("two")) return Person.SECOND;

        // First person dual or plural
        if (words[0].equals("we")) {
            if (words.length > 2) {
                if (words[2].charAt(0) == 'e') return Person.FIRSTEXCL;
                if (words[2].charAt(0) == 'i') return Person.FIRSTINCL;
            }
        }

        // Second person
        if (words[0].equals("you")) return Person.SECOND;

        // Third person
        if (words[0].equals("he") ^ words[0].equals("she") ^ words[0].equals("they")) return Person.THIRD;
    ...
    private static Gender getGender(String[] words) {
        // First person singular
        if (words[0].equals("i") ^ words[0].equals("he") ^ words[0].equals("she")) return Gender.SINGULAR;

        // Accounts for "you two"
        if (words[1].equals("two")) return Gender.DUAL;

        // Third person singular
        if (words[0].equals("he") ^ words[0].equals("she")) return Gender.SINGULAR;
        ...
        switch (gender) {
            ...
            default:
                if (gender > 3) return Gender.PLURAL;
                return null; // Invalid
        }
    }
    ...
    private static Tense getTense(String[] words, Person p, Gender g) {
        ...
        if ((p == Person.FIRSTEXCL ^ p == Person.THIRD) && g == Gender.SINGULAR) {
            ...
            if (words[1].equals("will")) {
                ...
                if (simplePresentAndFuture.contains(words[2]) ^ (words[2].equals("be") && progressive.contains(words[3]))) return Tense.FUTURE;
            }
            
            // Past simple XOR progressive
            if (simplePast.contains(words[1]) ^ (words[1].equals("was") && progressive.contains(words[2]))) return Tense.PAST;

            // Present first
            if (p == Person.FIRSTEXCL) {
                // Simple XOR progressive
                if (simplePresentAndFuture.contains(words[1]) ^ (words[1].equals("am") && progressive.contains(words[2]))) return Tense.PRESENT;
            } else { // Present third
                // Simple XOR progressive
                if (thirdPersonSingular.contains(words[1]) ^ (words[1].equals("is") && progressive.contains(words[2]))) return Tense.PRESENT;
            }
            ...
            if (words[3 + o].equals("will")) {
                // Simple XOR progressive
                if (simplePresentAndFuture.contains(words[4 + o]) ^ (words[4 + o].equals("be") && progressive.contains(words[5 + o]))) return Tense.FUTURE;
            }
            
            // Past simple XOR progressive
            if (simplePast.contains(words[3 + o]) ^ (words[3 + o].equals("were") && progressive.contains(words[4 + o]))) return Tense.PAST;

            // Present simple XOR progressive
            if (simplePresentAndFuture.contains(words[3 + o]) ^ (words[3 + o].equals("are") && progressive.contains(words[4 + o]))) return Tense.PRESENT;
        }
